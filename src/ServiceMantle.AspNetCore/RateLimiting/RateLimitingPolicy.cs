...
internal static class RateLimitingPolicy
{
    ...
    internal static RateLimitPartition<string> ManagementPartition(HttpContext context)
    {
        ...
        var entry = context.GetEndpoint()?.Metadata.GetMetadata<ManagementEntryMetadata>();
        var clientPartition = entry is not null && Enum.IsDefined(entry.Kind) &&
            ManagementEntryDefaults.Get(entry.Kind).AnonymousClientPartition;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            type = ProblemDetailsDefaults.CreateTypeUri(
                RateLimitingDefaults.RejectedErrorCode),
            title = "Too many requests.",
            status = StatusCodes.Status429TooManyRequests,
            correlationId,
            errorCode = RateLimitingDefaults.RejectedErrorCode,
        });
