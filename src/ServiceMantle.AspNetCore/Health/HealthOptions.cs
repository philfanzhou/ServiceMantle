...
internal sealed record HealthRegistration(
    TimeSpan ProbeTimeout,
    TimeSpan ContributorTimeout);

internal sealed class HealthStartupValidator(
    IEnumerable<HealthRegistration> registrations,
    IServiceProvider serviceProvider) : IHostedService
{
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (timeout < HealthOptions.MinimumProbeTimeout ||
            timeout > HealthOptions.MaximumProbeTimeout)
        {
            throw new InvalidOperationException(
        ...
        if (contributorTimeout < HealthOptions.MinimumContributorTimeout ||
            contributorTimeout > HealthOptions.MaximumContributorTimeout)
        {
            throw new InvalidOperationException(
