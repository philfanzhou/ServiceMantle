...
public sealed class SensitiveHeaderRegistry
{
    private readonly IEnumerable<SensitiveHeaderRegistration> registrations;
    private readonly object sync = new();
    private volatile FrozenSet<string>? snapshot;

    internal SensitiveHeaderRegistry(
        IEnumerable<SensitiveHeaderRegistration> registrations)
    {
    ...
    internal FrozenSet<string> GetRequiredSnapshot()
    {
        ...
        lock (sync)
        {
            ...
            SensitiveHeaderRegistration[] materialized;
            try
            {
            ...
            foreach (var registration in materialized)
            {
                ...
                catch
                {
                    throw Failure(
                        WellKnownSensitiveHeaderConfigurationErrorCodes.EnumerationFailed,
                        nameof(SensitiveHeadersOptions.DeniedHeaderNames));
                }
    ...
    private static SensitiveHeaderConfigurationException Failure(
        string errorCode,
