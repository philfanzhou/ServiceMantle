...
internal sealed class PhaseGateState(IEnumerable<PhaseGateRegistration> registrations)
{
    private IEndpointRouteBuilder? endpoints;
    private int useCount;
    internal PhaseGateRegistration GetConfiguration()
    {
        PhaseGateRegistration? baseline = null;
        foreach (var item in registrations)
        {
            var normalized = new PhaseGateRegistration(Normalize(item.Prefix), item.Timeout);
    ...
    internal void Validate()
    {
        ...
        foreach (var endpoint in endpoints.DataSources.SelectMany(source => source.Endpoints).OfType<RouteEndpoint>())
        {
            ...
            if (markers.Count == 1)
            {
                ...
                if (markers[0].Surface == ManagementSurface.Management &&
                    ...
                    throw Failure();
                if (markers[0].Surface == ManagementSurface.Status && !ReadOnly(endpoint)) throw Failure();
            }
            else if (Under(path, configuration.Prefix)) throw Failure();
        }
    }

    internal static bool Matches(string path, string prefix, ManagementSurface surface) => surface switch
    {
        ManagementSurface.Status => Under(path, prefix + "/status"),
        ManagementSurface.Bootstrap => Under(path, prefix + "/bootstrap"),
        ManagementSurface.Setup => Under(path, prefix + "/setup"),
        ManagementSurface.Management => Under(path, prefix) &&
...
internal sealed class PhaseGateStartupValidator(PhaseGateState state) : IHostedService
{
