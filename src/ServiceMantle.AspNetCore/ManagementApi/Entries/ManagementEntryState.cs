...
/// the host starts.
/// </summary>
internal sealed class ManagementEntryState(IServiceProvider services)
{
    private readonly List<IEndpointRouteBuilder> builders = [];

    internal string GetRootPath() =>
        (services.GetService<ManagementApiState>() ?? throw MissingCapability())
            .GetRootPath();
    ...
    internal async Task ValidateAsync(CancellationToken cancellationToken)
    {
        ...
        var seen = new HashSet<ManagementEntryKind>();
    ...
    private List<RouteEndpoint> Collect()
    {
        ...
        foreach (var builder in builders)
        {
            if (builder is not IApplicationBuilder application ||
                !PipelineComposition.IsCompleted(application))
            {
            ...
            foreach (var endpoint in builder.DataSources
                ...
                .Where(endpoint =>
                    endpoint.Metadata.GetOrderedMetadata<ManagementEntryMetadata>().Count > 0))
            {
    ...
    private static ManagementEntryDefinition Validate(RouteEndpoint endpoint, string root)
    {
        ...
        if (endpoint.Metadata.GetMetadata<ManagementApiMetadata>() is not null ||
            ...
            definition.RequiresUnsafeRequestHeader !=
                (endpoint.Metadata.GetMetadata<UnsafeRequestGuardMetadata>() is not null))
        {
            throw Failure();
        }
    ...
    private sealed record ValidatedEntry(
        ...
        ManagementEntryDefinition Definition);
...
internal sealed class ManagementEntryStartupValidator(
    ManagementEntryState state) : IHostedService
{
