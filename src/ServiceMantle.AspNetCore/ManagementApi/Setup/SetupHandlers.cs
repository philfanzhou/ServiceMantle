...
/// </para>
/// </remarks>
internal static class SetupHandlers
{
    ...
    internal static async Task<IResult> StatusAsync(HttpContext context)
    {
        ...
        return state switch
        {
            null => SetupResult.Unavailable,
            { IsCompleted: true } => SetupResult.Completed,
            _ => SetupResult.Pending,
        };
    }
        ...
        SetupExecutor executor)
    {
        ...
        try
        {
            code = await SetupRequestParser.ParseAsync(context).ConfigureAwait(false);
        }
        catch
        {
            ObserveCallerCancellation(context);
