...
/// </remarks>
internal sealed class ServiceMantleBootstrapResult : IResult
{
    /// <summary>The created file, which only a restart activates.</summary>
    internal static readonly ServiceMantleBootstrapResult Created = Json(
        StatusCodes.Status201Created);

    /// <summary>The replaced file, which only a restart activates.</summary>
    internal static readonly ServiceMantleBootstrapResult Updated = Json(
        StatusCodes.Status200OK);

    /// <summary>The single rejection every unusable credential shares.</summary>
    internal static readonly ServiceMantleBootstrapResult CredentialInvalid = new(
        Encoding.UTF8.GetBytes(
            "{\"errorCode\":\"" + ServiceMantleBootstrapMapping.CredentialInvalidErrorCode + "\"}"),
        StatusCodes.Status401Unauthorized);

    /// <summary>The single rejection every store, validator, or internal failure shares.</summary>
    internal static readonly ServiceMantleBootstrapResult Unavailable = new(
        Encoding.UTF8.GetBytes(
            "{\"errorCode\":\"" + ServiceMantleBootstrapMapping.UnavailableErrorCode + "\"}"),
        StatusCodes.Status503ServiceUnavailable);

    private readonly byte[] body;
    private readonly int statusCode;

    private ServiceMantleBootstrapResult(byte[] body, int statusCode)
    {
        this.body = body;
        this.statusCode = statusCode;
    }

    private static ServiceMantleBootstrapResult Json(int statusCode) =>
