...
/// </remarks>
public sealed class ServiceMantleManagementApiOptions
{
    /// <summary>
    /// Gets or sets the absolute versioned management root, defaulting to <c>/management/v1</c>.
    /// </summary>
    /// <remarks>
    /// The root must end with an independent <see cref="ServiceMantleManagementApiDefaults.VersionSegment"/>
    /// segment, for example <c>/ops/admin/v1</c>. Any other value fails before the host starts.
    /// </remarks>
    public string RootPath { get; set; } = ServiceMantleManagementApiDefaults.DefaultRootPath;

    /// <summary>
    /// Gets or sets the phase gate's asynchronous observation timeout for this root.
    /// </summary>
    /// <remarks>
    /// This is the existing <see cref="ServiceMantlePhaseGateOptions.SnapshotTimeout"/>; it is
    ...
    /// </remarks>
    public TimeSpan SnapshotTimeout { get; set; } = new ServiceMantlePhaseGateOptions().SnapshotTimeout;
}
