...
internal sealed record ManagementApiMetadata;

internal sealed class ManagementApiState(
    IEnumerable<ManagementApiRegistration> registrations,
    IServiceProvider services)
{
    ...
    internal string GetRootPath()
    {
        ...
        foreach (var registration in registrations)
        {
            ...
            try
            {
                ...
                normalized = PhaseGateState.Normalize(registration.RootPath);
            }
            ...
            if (!normalized.EndsWith('/' + ManagementApiDefaults.VersionSegment, StringComparison.Ordinal) ||
    ...
    internal async Task ValidateAsync(CancellationToken cancellationToken)
    {
        ...
        if (application is null || !PipelineComposition.IsCompleted(application)) throw MissingCapability();
        foreach (var endpoint in endpoints.DataSources
            .SelectMany(source => source.Endpoints)
            .OfType<RouteEndpoint>()
            .Where(endpoint => endpoint.Metadata.GetMetadata<ManagementApiMetadata>() is not null))
        {
    ...
    private static void Validate(RouteEndpoint endpoint, string root)
    {
        ...
        if (markers.Count != 1 || markers[0].Surface != ManagementSurface.Management ||
            !PhaseGateState.Under(endpoint.RoutePattern.RawText ?? "", root) ||
    ...
    private async Task ValidateCapabilitiesAsync(CancellationToken cancellationToken)
    {
        ...
        if (services.GetService<SecurityResponseHeadersRegistration>() is null ||
...
internal sealed class ManagementApiStartupValidator(ManagementApiState state) : IHostedService
{
