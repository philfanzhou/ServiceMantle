...
/// </para>
/// </remarks>
internal sealed class ManagementSessionBodyAdmission : IDisposable
{
    /// <summary>The most bytes this helper ever reads: one past the admitted envelope.</summary>
    internal const int ProbeLength =
        (int)ManagementSessionMapping.MaximumLoginBodyLength + 1;
    ...
    private ManagementSessionBodyAdmission(
        ManagementSessionBodyStatus status,
        byte[]? rented = null,
        int length = 0)
    {
        Status = status;
    ...
    internal enum ManagementSessionBodyStatus
    {
    ...
    internal ManagementSessionBodyStatus Status { get; }
    ...
    /// request token.
    /// </remarks>
    internal static async ValueTask<ManagementSessionBodyAdmission> ReadAsync(
        HttpRequest request,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (count < ProbeLength)
            {
                ...
                if (read == 0)
                {
                    ...
                    return new ManagementSessionBodyAdmission(
                        ManagementSessionBodyStatus.Admitted,
                        buffer,
                        count);
                }
            ...
            return Release(buffer, ManagementSessionBodyStatus.Rejected);
        }
        ...
        catch
        {
            ...
            return Release(buffer, ManagementSessionBodyStatus.Unavailable);
        }
    }
    ...
    internal void Install(HttpContext context)
    {
        ...
        if (Status != ManagementSessionBodyStatus.Admitted || installed is not null)
        {
            return;
        }
    ...
    private static ManagementSessionBodyAdmission Release(
        byte[] buffer,
        ManagementSessionBodyStatus status)
    {
        ArrayPool<byte>.Shared.Return(buffer, clearArray: true);
        return new ManagementSessionBodyAdmission(status);
    }
