...
public static class ServiceMantleMetricsBuilderExtensions
{
    ...
    /// instruments. No custom tags are accepted. Resolve ServiceMetrics to publish observed
    ...
    public static ServiceMantleBuilder AddServiceMantleMetrics(this ServiceMantleBuilder builder)
    {
        ...
        if (builder.Services.Any(descriptor => descriptor.ServiceType == typeof(ServiceMetrics)))
        {
            return builder;
        }
        builder.Services.AddSingleton(_ => new ServiceMetrics());
        builder.Services.AddOpenTelemetry().WithMetrics();
        builder.Services.ConfigureOpenTelemetryMeterProvider((services, provider) =>
        {
            var publisher = services.GetRequiredService<ServiceMetrics>();
            ...
            provider.SetResourceBuilder(ServiceMantleOpenTelemetryBuilderExtensions.CreateResource(identity))
                ...
                .AddView(instrument =>
                {
                    if (instrument.Meter.Name != ServiceMetrics.MeterName) return null;
                    if (!ReferenceEquals(instrument.Meter, publisher.Meter)) return MetricStreamConfiguration.Drop;
                    return instrument.Name switch
                    {
                        ServiceMetrics.ServiceInfoName => new MetricStreamConfiguration
                        {
                            TagKeys = [],
                            CardinalityLimit = 1
                        },
                        ServiceMetrics.InstallationPhaseName => new MetricStreamConfiguration
                        {
                            TagKeys = ["phase"],
                            CardinalityLimit = 4
                        },
