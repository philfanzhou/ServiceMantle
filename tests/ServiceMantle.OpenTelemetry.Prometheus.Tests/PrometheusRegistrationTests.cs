...
public sealed class PrometheusRegistrationTests
{
    ...
    public async Task Enabled_registration_requires_policy_name()
    {
        ...
        var exception = await Assert.ThrowsAsync<PrometheusConfigurationException>(() =>
    ...
    public async Task Invalid_path_fails_when_host_starts(string path)
    {
        ...
        Assert.Equal(WellKnownPrometheusErrorCodes.InvalidEndpointPath, exception.ErrorCode);
    ...
    public async Task Management_prefix_conflict_fails_when_host_starts()
    {
        ...
        Assert.Equal(WellKnownPrometheusErrorCodes.EndpointPathConflict, exception.ErrorCode);
    ...
    public async Task Equivalent_registrations_are_idempotent_and_conflicts_fail_at_startup()
    {
        ...
        var conflictException =
            await Assert.ThrowsAsync<PrometheusConfigurationException>(() =>
    ...
    public async Task Fifth_concurrent_scrape_is_rejected_immediately()
    {
        ...
        var firstFour = Enumerable.Range(0, PrometheusDefaults.MaximumConcurrentScrapes)
            .Select(_ => SendAuthorizedAsync(client))
            .ToArray();
        Assert.True(collectionEntered.Wait(TimeSpan.FromSeconds(2), testCancellationToken));
        var state = application.Services.GetRequiredService<PrometheusEndpointState>();
    ...
    public async Task Request_and_host_cancellation_propagate_and_stopping_rejects_new_scrapes()
    {
        ...
        var requestGate = new PrometheusScrapeGate(
            async context =>
            {
        ...
        var inFlightGate = new PrometheusScrapeGate(
            async context =>
            {
                try
                {
        ...
        var invoked = false;
        var stoppingGate = new PrometheusScrapeGate(
            _ =>
            {
                invoked = true;
    ...
    public async Task Internal_failure_before_response_start_returns_empty_503()
    {
        ...
        await application.StartAsync(TestContext.Current.CancellationToken);
        var state = application.Services.GetRequiredService<PrometheusEndpointState>();
        var gate = new PrometheusScrapeGate(
    ...
    public async Task Fixed_official_exporter_options_are_applied()
    {
        ...
        builder.Services.Configure<PrometheusAspNetCoreOptions>(options =>
        {
            ...
            options.MaxScrapeResponseSizeBytes =
                PrometheusDefaults.MaximumResponseSizeBytes + 1;
        ...
        var options = application.Services
            ...
            .CurrentValue;
        Assert.Equal(PrometheusDefaults.EndpointPath, options.ScrapeEndpointPath);
        Assert.Equal(
            PrometheusDefaults.MaximumResponseSizeBytes,
            options.MaxScrapeResponseSizeBytes);
    ...
    private static WebApplication CreateApplication(
        Action<PrometheusOptions> configure,
    ...
    private static void Enable(PrometheusOptions options)
    {
        options.Enabled = true;
        options.AuthorizationPolicyName = PolicyName;
    }
