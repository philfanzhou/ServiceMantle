...
public sealed class OtlpRegistrationTests
{
    ...
    public async Task Trace_and_metric_exporters_are_independently_enabled(bool tracing)
    {
        ...
        {
            var signal = tracing
                ? (OtlpSignalOptions)options.Traces
                : options.Metrics;
            signal.Enabled = true;
    ...
    [Theory]
    [InlineData("protocol", WellKnownOtlpErrorCodes.InvalidProtocol)]
    [InlineData("missing-endpoint", WellKnownOtlpErrorCodes.EndpointRequired)]
    ...
    [InlineData("delay-high", WellKnownOtlpErrorCodes.InvalidBatchDelay)]
    [InlineData("queue-low", WellKnownOtlpErrorCodes.InvalidQueueSize)]
    [InlineData("queue-high", WellKnownOtlpErrorCodes.InvalidQueueSize)]
    [InlineData("batch-low", WellKnownOtlpErrorCodes.InvalidBatchSize)]
    [InlineData("batch-high", WellKnownOtlpErrorCodes.InvalidBatchSize)]
    [InlineData("batch-over-queue", WellKnownOtlpErrorCodes.InvalidBatchSize)]
        ...
        string expectedCode)
    {
        ...
        serviceMantle.AddOpenTelemetryOtlpExporter(options =>
        {
            ...
            switch (scenario)
            {
                case "protocol": traces.Protocol = (OtlpProtocol)999; break;
        ...
        var exception = await Assert.ThrowsAsync<OtlpConfigurationException>(() =>
    ...
    public async Task Missing_or_throwing_authentication_is_safe_and_fails_at_startup()
    {
        ...
        Assert.Equal(WellKnownOtlpErrorCodes.AuthenticationMissing, exception.ErrorCode);
    ...
    public async Task Authentication_is_resolved_once_and_official_options_receive_the_header_safely()
    {
        ...
        Assert.DoesNotContain(token, new OtlpOptions().ToString(), StringComparison.Ordinal);
    }
    ...
    public async Task Invalid_resolved_header_fails_without_echoing_name_or_value()
    {
        ...
        Assert.Equal(WellKnownOtlpErrorCodes.AuthenticationInvalid, exception.ErrorCode);
    ...
    public async Task Conflicting_repeated_registration_fails_before_export()
    {
        ...
        Assert.Equal(WellKnownOtlpErrorCodes.ConflictingRegistration, exception.ErrorCode);
    }
    ...
    private static void EnableTrace(OtlpOptions options, string endpoint)
    {
        options.Traces.Enabled = true;
        options.Traces.Endpoint = new Uri(endpoint);
    }
