...
public sealed class CoreOptionalCompositionTests
{
    ...
    public async Task All_eight_subsets_run_real_HTTP_and_own_only_selected_capabilities(bool authentication, bool health, bool logging)
    {
        ...
        if (authentication)
        {
            ...
            {
                (null, 401, ManagementSessionDefaults.UnauthenticatedErrorCode),
                (ManagementPermission.Read, 403, ManagementSessionDefaults.ForbiddenErrorCode),
    ...
    private sealed class Composition : IAsyncDisposable
    {
        ...
        internal SerilogRuntime? Runtime { get; }
            ...
            bool duplicate = false, string? failure = null, ManualResetEventSlim? sinkRelease = null, bool throwOnDispose = false)
        {
            ...
            if (health) mantle.AddServiceMantleHealthEndpoints(options => options.ProbeTimeout =
                ...
                sourceMode == "timeout" ? HealthOptions.MinimumProbeTimeout : TimeSpan.FromSeconds(10));
            if (duplicate)
            {
            ...
            if (logging && !console) builder.Services.Replace(ServiceDescriptor.Singleton<ISerilogSinkFactory>(Sink));
            App = builder.Build();
            Runtime = App.Services.GetService<SerilogRuntime>();
            ...
            {
                var headers = context.RequestServices.GetRequiredService<RequestHeaderDiagnosticProjector>().Project(context.Request.Headers);
                context.RequestServices.GetRequiredService<ILogger<CoreOptionalCompositionTests>>()
        ...
        internal async Task AssertCapabilitiesAsync(bool authentication, bool health, bool logging)
        {
            ...
            if (authentication) Assert.Equal(ManagementSessionDefaults.AuthenticationScheme,
            ...
            Assert.Equal(logging ? 1 : 0, App.Services.GetServices<IHostedService>().Count(service => service is SerilogLifecycle));
            Assert.Equal(health ? 1 : 0, App.Services.GetServices<IHostedService>().Count(service => service.GetType().Name == "HealthStartupValidator"));
        }
        ...
        internal async Task<HttpResponseMessage> SendAsync(string path, ManagementPermission? permission = null)
        {
            ...
            if (permission is not null)
            {
                ...
                request.Headers.Add("Cookie", ManagementSessionDefaults.CookieName + "=" + options.TicketDataFormat.Protect(ticket));
            }
    ...
    private sealed class ControlledSink(ManualResetEventSlim? release, bool throwOnDispose)
        : ILogEventSink, IDisposable, ISerilogSinkFactory
    {
        ...
        public ILogEventSink Create(SerilogConfiguration configuration, ILogFieldSanitizer sanitizer)
        {
            Interlocked.Increment(ref Created);
            return new SanitizingSink(sanitizer,
