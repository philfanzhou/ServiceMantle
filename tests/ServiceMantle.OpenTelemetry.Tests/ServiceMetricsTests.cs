...
public sealed class ServiceMetricsTests
{
    [Fact]
    public async Task Fixed_contract_has_five_series_and_only_explicit_phase_tags()
    {
        ...
        var publisher = host.Services.GetRequiredService<ServiceMetrics>();
        var provider = host.Services.GetRequiredService<MeterProvider>();
        Assert.Single(host.Services.GetServices<ServiceMetrics>());
        ...
        Assert.All(exporter.Points, point =>
        {
            ...
            Assert.Equal(ServiceMetrics.MeterName, point.MeterName);
            Assert.Equal(ServiceMetrics.MeterVersion, point.MeterVersion);
    ...
    public async Task Independent_hosts_and_forged_same_name_meters_cannot_cross_publish()
    {
        ...
        using var external = new Meter(ServiceMetrics.MeterName, ServiceMetrics.MeterVersion);
        external.CreateObservableGauge(ServiceMetrics.InstallationPhaseName, () =>
        ...
        first.Services.GetRequiredService<ServiceMetrics>().SetPhase(ServiceStartupPhase.PendingSetup);
        second.Services.GetRequiredService<ServiceMetrics>().SetPhase(ServiceStartupPhase.Completed);
    ...
    public async Task Concurrent_phase_updates_and_collections_remain_one_hot_and_bounded()
    {
        ...
        try
        {
            for (var index = 0; index < 100; index++)
            {
                ...
                var phases = exporter.Points.Where(point => point.Name == ServiceMetrics.InstallationPhaseName).ToArray();
    ...
    public async Task Disposing_one_host_closes_only_its_publisher()
    {
        ...
        await first.StartAsync(TestContext.Current.CancellationToken);
        await second.StartAsync(TestContext.Current.CancellationToken);
        var publisher = first.Services.GetRequiredService<ServiceMetrics>();
    ...
    private static void AssertPhase(MeterProvider provider, CaptureExporter exporter, string expected)
    {
        ...
        var info = Assert.Single(exporter.Points, point => point.Name == ServiceMetrics.ServiceInfoName);
    ...
    private sealed class CaptureExporter : BaseExporter<Metric>
    {
        ...
        public override ExportResult Export(in Batch<Metric> batch)
        {
            ...
            {
                if (metric.MeterName != ServiceMetrics.MeterName) continue;
