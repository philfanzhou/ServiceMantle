...
public sealed class RateLimitingTests
{
    ...
    {
        Assert.Equal("servicemantle.setup", RateLimitingDefaults.SetupPolicyName);
        Assert.Equal("servicemantle.management", RateLimitingDefaults.ManagementPolicyName);
        Assert.Equal("rate_limit.exceeded", RateLimitingDefaults.RejectedErrorCode);
    ...
    public async Task ManagementUsesAStableOperatorPartitionAndFallsBackForInvalidClaims()
    {
        ...
        var firstPartition = RateLimitingPolicy.ManagementPartition(firstContext);
        var secondPartition = RateLimitingPolicy.ManagementPartition(secondContext);
    ...
    public async Task RetryAfterIsRoundedUpAndCallerCancellationStaysCancellation()
    {
        ...
        await RateLimitingPolicy.OnRejectedAsync(
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            RateLimitingPolicy.OnRejectedAsync(
                new OnRejectedContext
                {
    ...
    private static void ConfigureInvalid(RateLimitingOptions options, string scenario)
    {
        switch (scenario)
        {
    ...
    private static void ConfigureFast(RateLimitPolicyOptions options, int permits)
    {
