...
public sealed class SensitiveHeaderTests
{
    [Fact]
    public void Capability_is_opt_in_and_does_not_change_plain_AddServiceMantle()
    {
        ...
        Assert.Null(provider.GetService<SensitiveHeaderRegistry>());
        Assert.Null(provider.GetService<RequestHeaderDiagnosticProjector>());
    ...
    public async Task Built_ins_cannot_be_removed_and_additions_merge_case_insensitively()
    {
        ...
        var registry = host.Services.GetRequiredService<SensitiveHeaderRegistry>();
        ...
        Assert.Equal(
            ["DeniedHeaderNames"],
            typeof(SensitiveHeadersOptions).GetProperties().Select(property => property.Name));
        Assert.Single(host.Services.GetServices<SensitiveHeaderRegistry>());
        Assert.Single(host.Services.GetServices<RequestHeaderDiagnosticProjector>());
        Assert.Single(host.Services.GetServices<StructuredLogSanitizer>());
        Assert.Single(
            host.Services.GetServices<IHostedService>(),
            service => service.GetType().Name == "SensitiveHeaderStartupValidator");
    ...
    public async Task Registry_drives_both_the_public_sanitizer_and_request_diagnostic_projection()
    {
        ...
        var projector = host.Services.GetRequiredService<RequestHeaderDiagnosticProjector>();
    ...
    public async Task All_HTTP_token_characters_are_supported_by_the_registry_and_sanitizer()
    {
        ...
        Assert.True(host.Services.GetRequiredService<SensitiveHeaderRegistry>()
    ...
    public async Task Enumeration_and_configuration_callback_failures_are_safe_startup_errors()
    {
        ...
        var enumerationFailure = await Assert.ThrowsAsync<
            SensitiveHeaderConfigurationException>(() =>
    ...
    public async Task Product_headers_and_third_party_tracing_remain_explicitly_outside_the_defaults()
    {
        ...
        Assert.False(defaultHost.Services.GetRequiredService<SensitiveHeaderRegistry>()
        ...
        Assert.Equal(
            StructuredLogSanitizer.RedactedValue,
            configuredHost.Services.GetRequiredService<RequestHeaderDiagnosticProjector>()
                .Project(headers)[productHeader]);
        await configuredHost.StopAsync(TestContext.Current.CancellationToken);
    }

    private static IHost Build(params Action<SensitiveHeadersOptions>[] registrations)
    {
        var builder = Host.CreateApplicationBuilder();
        var serviceMantle = builder.Services.AddServiceMantle(
            ServiceId.Parse("catalog"),
