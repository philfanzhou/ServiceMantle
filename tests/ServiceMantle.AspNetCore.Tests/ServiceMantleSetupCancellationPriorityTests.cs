...
/// </remarks>
public sealed class ServiceMantleSetupCancellationPriorityTests
{
    ...
    public async Task The_store_boundary_propagates_the_callers_cancellation(string outcome)
    {
        ...
        var completion = await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            await ServiceMantleSetupHandlers.CompleteAsync(context, executor.Execute));
    ...
    public async Task The_read_entry_store_boundary_propagates_the_callers_cancellation(string outcome)
    {
        ...
        var status = await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            await ServiceMantleSetupHandlers.StatusAsync(context));
    ...
    public async Task A_request_cancelled_before_the_handler_never_reaches_the_store()
    {
        ...
        var completion = await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            await ServiceMantleSetupHandlers.CompleteAsync(context, executor.Execute));
        var status = await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            await ServiceMantleSetupHandlers.StatusAsync(CreateContext(store, abort.Token, null)));
        ...
        string outcome)
    {
        ...
        var fromStore = await ServiceMantleSetupHandlers.CompleteAsync(
            storeContext,
            new RecordingExecutor().Execute);
        var fromBody = await ServiceMantleSetupHandlers.CompleteAsync(
            bodyContext,
            new RecordingExecutor().Execute);
        var fromExecutor = await ServiceMantleSetupHandlers.CompleteAsync(
            executorContext,
            executor.Execute);

        Assert.Same(ServiceMantleSetupResult.Unavailable, fromStore);
        Assert.Same(ServiceMantleSetupResult.Unavailable, fromBody);
        Assert.Same(ServiceMantleSetupResult.Unavailable, fromExecutor);
    }
    ...
    public async Task Without_caller_cancellation_the_executor_mapping_is_unchanged(string outcome)
    {
        ...
        var result = await ServiceMantleSetupHandlers.CompleteAsync(context, executor.Execute);
    ...
    public async Task Without_caller_cancellation_the_fixed_rejections_are_unchanged()
    {
        ...
        var replay = await ServiceMantleSetupHandlers.CompleteAsync(completed, executor.Execute);
        var rejected = await ServiceMantleSetupHandlers.CompleteAsync(invalidBody, executor.Execute);
        var unavailable = await ServiceMantleSetupHandlers.CompleteAsync(noStore, executor.Execute);
        var status = await ServiceMantleSetupHandlers.StatusAsync(pending);

        Assert.Equal("conflict", Describe(replay));
        Assert.Equal("invalid-request", Describe(rejected));
        Assert.Same(ServiceMantleSetupResult.Unavailable, unavailable);
        Assert.Same(ServiceMantleSetupResult.Pending, status);
        Assert.Equal(0, executor.Calls);
    }
    ...
    public async Task Two_concurrent_requests_never_share_a_token_or_a_result()
    {
        ...
        var cancelled = Task.Run(
            async () => await ServiceMantleSetupHandlers.CompleteAsync(
                cancelledContext,
        ...
        Assert.Same(ServiceMantleSetupResult.NoContent, result);
    ...
    private static string Describe(IResult result) =>
        ReferenceEquals(result, ServiceMantleSetupResult.NoContent) ? "no-content"
        : ReferenceEquals(result, ServiceMantleSetupResult.CredentialInvalid) ? "credential-invalid"
        : ReferenceEquals(result, ServiceMantleSetupResult.Unavailable) ? "unavailable"
        : ReferenceEquals(result, ServiceMantleSetupResult.Pending) ? "pending"
        : ReferenceEquals(result, ServiceMantleSetupResult.Completed) ? "completed"
        : ReferenceEquals(result, ServiceMantleManagementApiProblemResult.Conflict) ? "conflict"
        : ReferenceEquals(result, ServiceMantleManagementApiProblemResult.InvalidRequest)
            ? "invalid-request"
            : result.GetType().Name;
        ...
        Barrier? barrier = null)
    {
        ...
        internal ValueTask<ServiceMantleSetupCompletionResult> Execute(
            HttpContext context,
            SetupCode setupCode,
            CancellationToken cancellationToken)
        {
            ...
            return outcome switch
            {
                "committed" => ValueTask.FromResult(ServiceMantleSetupCompletionResult.Committed()),
                "credential-invalid" => ValueTask.FromResult(
                    ServiceMantleSetupCompletionResult.CredentialInvalid()),
                "conflict" => ValueTask.FromResult(ServiceMantleSetupCompletionResult.Conflict()),
                ...
                "null" => ValueTask.FromResult<ServiceMantleSetupCompletionResult>(null!),
                "timeout" => ValueTask.FromException<ServiceMantleSetupCompletionResult>(
                    new TimeoutException("internal timeout")),
                "foreign-cancellation" => ValueTask.FromException<ServiceMantleSetupCompletionResult>(
                    new OperationCanceledException(
                        "internal",
                        foreignToken == default ? new CancellationTokenSource().Token : foreignToken)),
                _ => ValueTask.FromException<ServiceMantleSetupCompletionResult>(
    ...
    private sealed class Host : IAsyncDisposable
    {
        ...
        {
            var root = ServiceMantleManagementApiDefaults.DefaultRootPath;
