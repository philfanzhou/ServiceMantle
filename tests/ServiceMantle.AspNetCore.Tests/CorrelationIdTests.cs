...
public sealed class CorrelationIdTests
{
    ...
    {
        Assert.Equal("x-correlation-id", ServiceHeaderNames.CorrelationId);
    ...
    public async Task ValidCallerValue_IsReusedVerbatimInSlotScopeAndResponse()
    {
        ...
        Assert.Equal(
            callerValue,
            Assert.Single(response.Headers[ServiceHeaderNames.CorrelationId]));
        Assert.Equal(callerValue, context.Request.Headers[ServiceHeaderNames.CorrelationId]);
    }
    ...
    public async Task RejectedHeader_GeneratesANewValueAndNeverEchoesTheRawInput(string shape)
    {
        ...
        var responseValue = Assert.Single(response.Headers[ServiceHeaderNames.CorrelationId]);
    ...
    public async Task ResponseHeader_CollapsesToASingleAssignedValue()
    {
        ...
        {
            context.Response.Headers.Append(ServiceHeaderNames.CorrelationId, "downstream-a");
            context.Response.Headers.Append(ServiceHeaderNames.CorrelationId, "downstream-b");
    ...
    public async Task AlreadyStartedResponse_StillEstablishesContextWithoutRewritingSentHeaders()
    {
        ...
        response.Headers[ServiceHeaderNames.CorrelationId] = "already-sent";
        var context = fixture.CreateContext(response, "caller-1");

        await fixture.SendAsync(context);

        Assert.Equal(0, response.RegisteredStartingCallbacks);
        Assert.Equal("already-sent", response.Headers[ServiceHeaderNames.CorrelationId]);
    ...
    public async Task MinimalHost_RoundTripsTheCorrelationIdOnlyWhenTheMiddlewareIsUsed()
    {
        ...
        callerRequest.Headers.TryAddWithoutValidation(
            ServiceHeaderNames.CorrelationId,
            "caller-round-trip");
        ...
        Assert.Equal(
            "caller-round-trip",
            Assert.Single(callerResponse.Headers.GetValues(ServiceHeaderNames.CorrelationId)));
        Assert.Equal(
            "caller-round-trip",
        ...
        var generated = Assert.Single(
            generatedResponse.Headers.GetValues(ServiceHeaderNames.CorrelationId));
        ...
        Assert.False(withoutMiddleware.Headers.Contains(ServiceHeaderNames.CorrelationId));
        Assert.Equal(
            "none",
    ...
    private sealed class PipelineFixture : IDisposable
    {
            ...
            params string[] headerValues)
        {
            ...
            if (headerValues.Length > 0)
            {
                ...
                store[ServiceHeaderNames.CorrelationId] = new StringValues(headerValues);
            }
