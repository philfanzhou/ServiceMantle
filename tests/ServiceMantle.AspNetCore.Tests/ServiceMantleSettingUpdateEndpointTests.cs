...
public sealed class ServiceMantleSettingUpdateEndpointTests
{
    ...
    public async Task Strict_json_rejections_are_fixed_and_have_no_partial_execution(string body)
    {
        ...
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        Assert.Contains(ServiceMantleManagementApiDefaults.InvalidRequestErrorCode, responseBody, StringComparison.Ordinal);
    ...
    public async Task Concurrent_requests_keep_scope_correlation_and_cancellation_isolated()
    {
        ...
        Assert.Equal(
            "update-first",
            Assert.Single(firstContext.Response.Headers[ServiceMantleHeaderNames.CorrelationId].ToArray()));
        Assert.Equal(
            "update-second",
            Assert.Single(secondContext.Response.Headers[ServiceMantleHeaderNames.CorrelationId].ToArray()));
    }
