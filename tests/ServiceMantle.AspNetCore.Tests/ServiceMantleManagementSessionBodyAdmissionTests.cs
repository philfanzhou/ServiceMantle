...
/// </remarks>
public sealed class ServiceMantleManagementSessionBodyAdmissionTests
{
    private const int Limit = (int)ServiceMantleManagementSessionMapping.MaximumLoginBodyLength;

    private const int Probe = ServiceMantleManagementSessionBodyAdmission.ProbeLength;

    private const string Sentinel = "sentinel-login-body-credential";

    private static readonly TimeSpan Budget = ServiceMantleManagementSessionOptions.MaximumLoginTimeout;
    ...
    public async Task A_body_read_that_spends_the_login_budget_never_reaches_the_adapter()
    {
        ...
        var outcome = await scope.LoginAsync(
            adapter,
            ServiceMantleManagementSessionOptions.MinimumLoginTimeout);
    ...
    private sealed class LoginScope : IDisposable
    {
        ...
        internal Task<IResult> RunAsync(
            ...
            ServiceMantleManagementSessionHandlers.LoginAsync(Context, adapter, budget ?? Budget);
