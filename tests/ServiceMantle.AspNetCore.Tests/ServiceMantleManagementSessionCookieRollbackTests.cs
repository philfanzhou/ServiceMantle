...
/// </remarks>
public sealed class ServiceMantleManagementSessionCookieRollbackTests
{
    ...
    public async Task A_rollback_that_cannot_be_applied_aborts_instead_of_answering(HeaderMode mode)
    {
        ...
        Assert.Same(ServiceMantleManagementSessionResult.Terminated, result);
    ...
    public async Task Two_concurrent_logins_never_restore_each_other_s_cookies()
    {
        ...
        {
            Append(context.Response, ServiceMantleManagementSessionDefaults.CookieName + "=granted; path=/");
        ...
        Assert.Same(ServiceMantleManagementSessionResult.NoContent, results[0]);
        Assert.Same(ServiceMantleManagementSessionResult.Unavailable, results[1]);
        string[] expected =
            [.. succeeding.Existing, ServiceMantleManagementSessionDefaults.CookieName + "=granted; path=/"];
    ...
    {
        var name = ServiceMantleManagementSessionDefaults.CookieName;
        switch (shape)
        {
    ...
    private static string[] CookiesFor(ExistingCookies existing) => existing switch
    {
        ...
        _ =>
        [
            ServiceMantleManagementSessionDefaults.CookieName + "=already-here; path=/; secure; httponly",
            "unrelated-a=1; path=/"
        ],
    };
        ...
        internal Task<HttpResponseMessage> LoginAsync(string? cookie = null)
        {
            var request = Create(
                ...
                ServiceMantleManagementApiDefaults.DefaultRootPath +
                    ServiceMantleManagementEntryDefaults.SessionLoginPath,
                cookie);
        ...
        internal Task<HttpResponseMessage> ReadSessionAsync(string cookie) =>
            client.SendAsync(
                Create(
                    HttpMethod.Get,
                    ServiceMantleManagementApiDefaults.DefaultRootPath +
                        ServiceMantleManagementEntryDefaults.SessionPath,
                    cookie),
                Token);
