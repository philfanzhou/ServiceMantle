...
public sealed class ProblemDetailsTests
{
    ...
    {
        Assert.Equal("urn:servicemantle:error:", ProblemDetailsDefaults.TypeUriPrefix);
        Assert.Equal("correlationId", ProblemDetailsDefaults.CorrelationIdExtensionName);
        Assert.Equal("errorCode", ProblemDetailsDefaults.ErrorCodeExtensionName);
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            ProblemDetailsDefaults.CreateTypeUri("catalog.request_invalid"));

        Assert.Throws<ArgumentException>(() =>
            ProblemDetailsDefaults.CreateTypeUri("Catalog Invalid"));
    }
    ...
    private sealed class PipelineFixture : IDisposable
    {
            ...
            CancellationToken requestAborted)
        {
            ...
            {
                requestHeaders[ServiceHeaderNames.CorrelationId] = correlationId;
            }
