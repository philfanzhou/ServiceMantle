...
public sealed class PhaseGateTests
{
    ...
    public async Task Configuration_or_endpoint_contract_conflicts_fail_startup(string scenario)
    {
        ...
        if (scenario == "wrong_surface") app.MapGet("/management/setup", () => "ok").WithServiceMantleManagementSurface(ManagementSurface.Bootstrap);
        if (scenario == "unknown_surface") app.MapGet("/management/setup", () => "ok").WithServiceMantleManagementSurface((ManagementSurface)999);
        if (scenario == "unmarked_management") app.MapGet("/management/unclassified", () => "ok");
        if (scenario == "status_post") app.MapPost("/management/status", () => "ok").WithServiceMantleManagementSurface(ManagementSurface.Status);
        if (scenario == "dynamic_management_child") app.MapGet("/management/{**rest}", () => "ok").WithServiceMantleManagementSurface(ManagementSurface.Management);
        if (scenario == "duplicate_route")
        {
            app.MapGet("/management/settings", () => "first").WithServiceMantleManagementSurface(ManagementSurface.Management);
            app.MapGet("/MANAGEMENT/settings/", () => "second").WithServiceMantleManagementSurface(ManagementSurface.Management);
        }
        var exception = await Assert.ThrowsAnyAsync<InvalidOperationException>(() => app.StartAsync(Token));
    ...
    private static WebApplication Build(Action<IServiceCollection>? services = null,
        Action<PhaseGateOptions>? configure = null, bool health = false, bool map = true,
        int useCount = 1, string? repeatPrefix = null)
    {
        ...
        if (map)
        {
            ...
            management.MapGet("/bootstrap", () => "bootstrap").WithServiceMantleManagementSurface(ManagementSurface.Bootstrap);
            management.MapGet("/setup", () => "setup").WithServiceMantleManagementSurface(ManagementSurface.Setup);
            management.MapGet("/settings", () => "settings").WithServiceMantleManagementSurface(ManagementSurface.Management);
        }
