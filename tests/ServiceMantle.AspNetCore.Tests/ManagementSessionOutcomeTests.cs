...
/// </remarks>
public sealed class ManagementSessionOutcomeTests
{
    private const string Sentinel = "sentinel-session-outcome-detail";

    private static readonly TimeSpan Budget = ManagementSessionOptions.MinimumLoginTimeout;
    ...
    public async Task An_expired_budget_never_signs_in_whatever_the_adapter_returned(AdapterOutcome outcome)
    {
        ...
        var result = await ManagementSessionHandlers.LoginAsync(
            scope.Context,
            async (_, token) =>
            {
    ...
    public async Task An_ordinary_sign_in_failure_is_fixed_unless_the_caller_already_cancelled()
    {
        ...
        var fixedResult = await ManagementSessionHandlers.LoginAsync(
            uncancelled.Context,
            (_, _) => ValueTask.FromResult(Authenticated()),
            ManagementSessionOptions.MaximumLoginTimeout);

        Assert.Same(ManagementSessionResult.Unavailable, fixedResult);
    ...
    public async Task An_ordinary_authenticate_failure_is_fixed_unless_the_caller_already_cancelled()
    {
        ...
        Assert.Same(
            ...
            await ManagementSessionHandlers.CurrentAsync(uncancelled.Context));
        ...
        var exception = await Assert.ThrowsAsync<OperationCanceledException>(
            () => ManagementSessionHandlers.CurrentAsync(cancelled.Context));
    ...
    public async Task An_ordinary_sign_out_failure_is_fixed_unless_the_caller_already_cancelled()
    {
        ...
        Assert.Same(
            ManagementSessionResult.Unavailable,
            await ManagementSessionHandlers.LogoutAsync(uncancelled.Context));
        ...
        var exception = await Assert.ThrowsAsync<OperationCanceledException>(
            () => ManagementSessionHandlers.LogoutAsync(cancelled.Context));
    ...
    public async Task An_uncancelled_login_within_its_budget_keeps_its_existing_result()
    {
        foreach (var (outcome, expected, expectedSignIns) in new[]
                 {
                     (AdapterOutcome.Authenticated, ManagementSessionResult.NoContent, 1),
                     (AdapterOutcome.Unauthenticated, ManagementSessionResult.Unauthenticated, 0),
                     (AdapterOutcome.Failed, ManagementSessionResult.Unavailable, 0),
                     (AdapterOutcome.Null, ManagementSessionResult.Unavailable, 0)
                 })
        {
            using var scope = new SessionScope();
    ...
    public async Task The_current_session_and_logout_keep_their_normal_results()
    {
        ...
        Assert.IsType<ManagementSessionResult>(
            await ManagementSessionHandlers.CurrentAsync(current.Context));
        Assert.Equal(1, current.Authentication.AuthenticateCalls);

        using var logout = new SessionScope();
        Assert.Same(
            ManagementSessionResult.NoContent,
            await ManagementSessionHandlers.LogoutAsync(logout.Context));
