...
public sealed class ManagementSessionEndpointTests
{
    ...
    private static string CookieName => ManagementSessionDefaults.CookieName;
    ...
    public async Task The_login_admission_envelope_is_enforced_before_the_adapter_runs()
    {
        ...
        var oversized = "{\"password\":\"" +
            new string('p', (int)ManagementSessionMapping.MaximumLoginBodyLength) + "\"}";
        ...
        Assert.True(oversized.Length > ManagementSessionMapping.MaximumLoginBodyLength);
    }
    ...
    public async Task A_body_at_the_admission_limit_still_reaches_the_adapter()
    {
        ...
        var padding = new string(
            'p',
            (int)ManagementSessionMapping.MaximumLoginBodyLength - prefix.Length - suffix.Length);

        using var response = await fixture.LoginAsync(body: prefix + padding + suffix);

        Assert.Equal(HttpStatusCode.NoContent, response.StatusCode);
        Assert.Equal(
            ManagementSessionMapping.MaximumLoginBodyLength,
