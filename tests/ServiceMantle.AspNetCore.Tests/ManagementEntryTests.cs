...
/// </summary>
public sealed class ManagementEntryTests
{
    ...
    public static TheoryData<ManagementEntryKind> Kinds
    {
        get
        {
            var data = new TheoryData<ManagementEntryKind>();
        ...
        ServiceDatabaseReadinessState database)
    {
        ...
        return kind switch
        {
            ManagementEntryKind.InstallationStatus => true,
    ...
    public async Task Anonymous_bootstrap_create_never_runs_once_the_service_is_configured()
    {
        ...
        using var response = await fixture.SendAsync(ManagementEntryKind.BootstrapCreate);
    ...
    [Theory]
    [InlineData(ManagementEntryKind.SetupStatus)]
    [InlineData(ManagementEntryKind.SetupComplete)]
    public async Task Setup_replay_reaches_the_handler_after_completion(ManagementEntryKind kind)
    {
        var source = new ManagementEntryHostFixture.SnapshotSource(ManagementEntryHostFixture.Ready);
        await using var fixture = await ManagementEntryHostFixture.StartAsync(source);
    ...
    public async Task Each_kind_only_accepts_its_fixed_path_and_method(ManagementEntryKind kind)
    {
        var phase = kind == ManagementEntryKind.BootstrapCreate
            ? ServiceStartupPhase.BootstrapConfiguration
    ...
    [Theory]
    [InlineData(ManagementEntryKind.BootstrapCreate)]
    [InlineData(ManagementEntryKind.SetupComplete)]
    [InlineData(ManagementEntryKind.SessionLogin)]
    [InlineData(ManagementEntryKind.SessionLogout)]
    [InlineData(ManagementEntryKind.BootstrapUpdate)]
    public async Task Unsafe_entries_require_exactly_one_fixed_request_header(
        ManagementEntryKind kind)
    {
        var source = new ManagementEntryHostFixture.SnapshotSource(
            kind == ManagementEntryKind.BootstrapCreate
    ...
    [Theory]
    [InlineData(ManagementEntryKind.InstallationStatus)]
    [InlineData(ManagementEntryKind.SetupStatus)]
    [InlineData(ManagementEntryKind.CurrentSession)]
    ...
    [InlineData(ManagementEntryKind.SetupComplete)]
    [InlineData(ManagementEntryKind.SessionLogin)]
    public async Task Anonymous_entries_run_without_any_cookie(ManagementEntryKind kind)
    {
        var source = new ManagementEntryHostFixture.SnapshotSource(
            kind == ManagementEntryKind.BootstrapCreate
                ? new ServiceHealthSnapshot(
    ...
    public async Task Session_entries_accept_a_legitimate_identity_without_admin()
    {
        ...
        using var logout = await fixture.SendAsync(ManagementEntryKind.SessionLogout, reader);
        using var current = await fixture.SendAsync(ManagementEntryKind.CurrentSession, reader);
        using var update = await fixture.SendAsync(ManagementEntryKind.BootstrapUpdate, reader);

        Assert.Equal(HttpStatusCode.OK, logout.StatusCode);
        Assert.Equal(HttpStatusCode.OK, current.StatusCode);
        // The same identity is still rejected by the administrator-only Bootstrap update.
        Assert.Equal(HttpStatusCode.Forbidden, update.StatusCode);
        Assert.Equal(0, fixture.Recorder.Calls(ManagementEntryKind.BootstrapUpdate));
    }
    ...
    public async Task Installation_status_stays_on_the_anonymous_client_partition()
    {
        ...
        Assert.Equal(4, fixture.Recorder.Calls(ManagementEntryKind.InstallationStatus));
    }
    ...
    public async Task Setup_partitioned_entries_share_the_setup_policy_quota()
    {
        ...
        using var first = await fixture.SendAsync(ManagementEntryKind.SetupStatus);
        using var second = await fixture.SendAsync(ManagementEntryKind.SetupComplete);
        using var third = await fixture.SendAsync(ManagementEntryKind.SessionLogin);

        Assert.Equal(HttpStatusCode.OK, first.StatusCode);
        Assert.Equal(HttpStatusCode.OK, second.StatusCode);
        Assert.Equal(HttpStatusCode.TooManyRequests, third.StatusCode);
        Assert.Equal(0, fixture.Recorder.Calls(ManagementEntryKind.SessionLogin));
    }
    ...
    public async Task Caller_cancellation_propagates_and_never_becomes_a_fixed_error()
    {
        ...
        var error = await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            fixture.SendAsync(ManagementEntryKind.InstallationStatus, abort: abort));
    ...
    public async Task Concurrent_entry_requests_stay_isolated()
    {
        ...
        Assert.Equal(6, fixture.Recorder.Calls(ManagementEntryKind.CurrentSession));
        Assert.Equal(6, fixture.Recorder.Calls(ManagementEntryKind.SetupStatus));
    ...
    public async Task A_phase_change_after_admission_does_not_recall_the_request()
    {
        ...
        await using var fixture = await ManagementEntryHostFixture.StartAsync(
            source,
            map: [ManagementEntryKind.CurrentSession]);
        fixture.Recorder.Hold = ManagementEntryKind.CurrentSession;
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal(1, fixture.Recorder.Calls(ManagementEntryKind.CurrentSession));
    }
    ...
    private static async Task<ManagementEntryHostFixture> CreateForFailureAsync(string scenario) => scenario switch
    {
        ...
        "second-surface" => await ManagementEntryHostFixture.CreateAsync(
            map: [ManagementEntryKind.SetupStatus],
            configureEntry: (_, entry) => entry.WithServiceMantleManagementSurface(
                ManagementSurface.Management)),
