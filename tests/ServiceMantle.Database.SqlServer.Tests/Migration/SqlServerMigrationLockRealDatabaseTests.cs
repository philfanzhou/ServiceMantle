...
public sealed class SqlServerMigrationLockRealDatabaseTests(
    SqlServerContainerFixture fixture) : IClassFixture<SqlServerContainerFixture>
{
    [Fact]
    public async Task Application_locks_contend_only_for_the_same_service_and_target_database()
    {
        var bootstrap = RequireBootstrap(fixture.PrimaryDatabaseName);
        var otherDatabaseBootstrap = RequireBootstrap(fixture.SecondaryDatabaseName);
        var provider = new SqlServerMigrationLockProvider();
        var serviceA = ServiceId.Parse($"sqlserver-a-{Guid.NewGuid():N}");
        var serviceB = ServiceId.Parse($"sqlserver-b-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Assert.DoesNotContain("Password", timeout.ToString(), StringComparison.OrdinalIgnoreCase);
            Assert.DoesNotContain(fixture.PrimaryDatabaseName, timeout.ToString(), StringComparison.Ordinal);
            ...
            await using var leaseB = await provider.AcquireAsync(
                ...
                TestContext.Current.CancellationToken);
            await using var otherDatabaseLease = await provider.AcquireAsync(
                serviceA,
                otherDatabaseBootstrap,
                TimeSpan.FromSeconds(2),
                TestContext.Current.CancellationToken);
            Assert.False(leaseB.LeaseLost.IsCancellationRequested);
            Assert.False(otherDatabaseLease.LeaseLost.IsCancellationRequested);
        }
        finally
        {
    ...
    [Fact]
    public async Task Caller_cancellation_cleans_up_the_waiting_session_for_reacquisition()
    {
        var bootstrap = RequireBootstrap(fixture.PrimaryDatabaseName);
        var serviceId = ServiceId.Parse($"sqlserver-cancel-{Guid.NewGuid():N}");
        var provider = new SqlServerMigrationLockProvider();
        var holder = await provider.AcquireAsync(
            serviceId,
            bootstrap,
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task Ambient_transaction_is_not_enlisted_and_rollback_does_not_own_the_lock()
    {
        var bootstrap = RequireBootstrap(fixture.PrimaryDatabaseName);
        var serviceId = ServiceId.Parse($"sqlserver-ambient-{Guid.NewGuid():N}");
        var provider = new SqlServerMigrationLockProvider();
    ...
    public async Task Holding_session_termination_fails_closed_before_the_next_stage(int stageValue)
    {
        var bootstrap = RequireBootstrap(fixture.PrimaryDatabaseName);
        ...
        var orchestration = CreateOrchestrator(lockProvider, executor)
            .OrchestrateMigrationAsync(
                ServiceId.Parse($"sqlserver-stage-{stageValue}-{Guid.NewGuid():N}"),
                bootstrap,
        ...
        await stageReached.Task.WaitAsync(
            TimeSpan.FromSeconds(10),
            TestContext.Current.CancellationToken);
        var lease = Assert.IsType<SqlServerMigrationLock>(lockProvider.AcquiredLease);

        await KillSessionAsync(lease.SessionId);
        ...
        Assert.True(detection.Elapsed <= SqlServerMigrationLock.LeaseLossDetectionBound);
    ...
    public async Task Two_real_orchestrators_execute_once_and_the_contender_rechecks_under_the_lock()
    {
        ...
        try
        {
            var serviceId = ServiceId.Parse($"sqlserver-orchestrator-{Guid.NewGuid():N}");
            ...
            var firstExecutor = new RealStateExecutor(
                RequireConnectionString(fixture.PrimaryDatabaseName),
                table,
                executionStarted,
                allowExecution);
            var secondExecutor = new RealStateExecutor(
                ...
                executionStarted,
                allowExecution);
            var firstProvider = new SqlServerMigrationLockProvider();
            var secondProvider = new SqlServerMigrationLockProvider(
                new AcquireSignallingOperations(
                    new SqlServerMigrationLockOperations(),
            ...
            var held = await Assert.ThrowsAsync<DatabaseMigrationLockException>(async () =>
                await new SqlServerMigrationLockProvider().AcquireAsync(
                    serviceId,
                    bootstrap,
                    TimeSpan.FromMilliseconds(250),
                    TestContext.Current.CancellationToken));
        ...
        {
            await ExecuteAsync(
                fixture.PrimaryDatabaseName,
                $"DROP TABLE IF EXISTS [dbo].[{table}]");
        }
    }

    private BootstrapDatabaseConfiguration RequireBootstrap(string databaseName) => new(
        WellKnownDatabaseProviderIds.SqlServer,
        "16.0",
        RequireConnectionString(databaseName));

    private string RequireConnectionString(string databaseName)
    {
    ...
    private async Task KillSessionAsync(int sessionId)
    {
        ...
        command.CommandText = $"KILL {sessionId.ToString(CultureInfo.InvariantCulture)}";
    ...
    private async Task CreateMigrationStateAsync(string table)
    {
        await ExecuteAsync(
            fixture.PrimaryDatabaseName,
            $"CREATE TABLE [dbo].[{table}] ([state] nvarchar(20) NOT NULL, [execution_count] int NOT NULL)");
        await ExecuteAsync(
            fixture.PrimaryDatabaseName,
            $"INSERT INTO [dbo].[{table}] ([state], [execution_count]) VALUES (N'pending', 0)");
    }
    ...
    private async Task<int> ReadExecutionCountAsync(string table)
    {
        ...
        await connection.OpenAsync(TestContext.Current.CancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = $"SELECT [execution_count] FROM [dbo].[{table}]";
        return Convert.ToInt32(
            await command.ExecuteScalarAsync(TestContext.Current.CancellationToken),
            CultureInfo.InvariantCulture);
    }

    private async Task ExecuteAsync(string databaseName, string sql)
    {
        await using var connection = new SqlConnection(RequireConnectionString(databaseName));
        await connection.OpenAsync(TestContext.Current.CancellationToken);
        await using var command = connection.CreateCommand();
    ...
    {
        private readonly SqlServerMigrationLockProvider inner = new();
        ...
        TaskCompletionSource allowExecution) : IDatabaseMigrationExecutor
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await using var command = connection.CreateCommand();
            command.CommandText = $"SELECT [state] FROM [dbo].[{table}]";
        ...
        public async ValueTask ExecuteAsync(CancellationToken cancellationToken = default)
        {
            ...
            await using var command = connection.CreateCommand();
            command.CommandText =
                $"UPDATE [dbo].[{table}] SET [state] = N'current', " +
    ...
    private sealed class AcquireSignallingOperations(
        ISqlServerMigrationLockOperations inner,
        TaskCompletionSource acquireStarted) : ISqlServerMigrationLockOperations
    {
    ...
    private sealed class AcquireSignallingSession(
        ISqlServerMigrationLockSession inner,
        TaskCompletionSource acquireStarted) : ISqlServerMigrationLockSession
    {
        ...
        public ValueTask ProbeLeaseAsync(
            string resourceName,
            CancellationToken cancellationToken) =>
            inner.ProbeLeaseAsync(resourceName, cancellationToken);

        public ValueTask<int> ReleaseLockAsync(
            string resourceName,
            CancellationToken cancellationToken) =>
            inner.ReleaseLockAsync(resourceName, cancellationToken);
...
public sealed class SqlServerContainerFixture : IAsyncLifetime
{
    ...
    private const string ImageVariable = "SERVICEMANTLE_SQLSERVER_IMAGE";

    private MsSqlContainer? container;

    public string PrimaryDatabaseName { get; } = "servicemantle_lock";

    public string SecondaryDatabaseName { get; } = "servicemantle_lock_secondary";
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        await CreateDatabaseAsync(PrimaryDatabaseName);
        await CreateDatabaseAsync(SecondaryDatabaseName);
    }
