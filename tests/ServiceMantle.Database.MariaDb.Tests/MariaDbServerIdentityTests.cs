...
public sealed class MariaDbServerIdentityTests(MariaDbIdentityFixture fixture)
    : ServerIdentityPreparationTests, IClassFixture<MariaDbIdentityFixture>
{
    protected override RealDatabaseProvider DatabaseProvider => RealDatabaseProvider.MariaDb;
    protected override IDatabaseTargetPreparationProvider Provider => new MariaDbDatabaseTargetPreparationProvider();
...
public sealed class MariaDbIdentityFixture : IAsyncLifetime
{
    private readonly List<MariaDbContainer> containers = [];
    public string? First { get; private set; }
    public string? Second { get; private set; }

    public async ValueTask InitializeAsync()
    {
        if (!RealDatabaseTestEnvironment.IsRequired(RealDatabaseProvider.MariaDb)) return;
        try
        {
            for (var i = 0; i < 2; i++)
            {
                var instance = new MariaDbBuilder("mariadb:11.4.8")
