...
public sealed class MariaDbMigrationLockRealDatabaseTests(
    MariaDbContainerFixture fixture) : IClassFixture<MariaDbContainerFixture>
{
    [Fact]
    public async Task Named_locks_contend_only_for_the_same_service_and_release_for_reacquisition()
    {
        ...
        var provider = new MariaDbMigrationLockProvider();
        var serviceA = ServiceId.Parse($"mariadb-a-{Guid.NewGuid():N}");
        var serviceB = ServiceId.Parse($"mariadb-b-{Guid.NewGuid():N}");
    ...
    {
        var bootstrap = RequireBootstrap();
        var serviceId = ServiceId.Parse($"mariadb-cancel-{Guid.NewGuid():N}");
        var provider = new MariaDbMigrationLockProvider();
    ...
    public async Task Holding_connection_termination_fails_closed_before_the_next_stage(int stageValue)
    {
        ...
        var orchestration = CreateOrchestrator(lockProvider, executor)
            .OrchestrateMigrationAsync(
                ServiceId.Parse($"mariadb-stage-{stageValue}-{Guid.NewGuid():N}"),
                bootstrap,
        ...
        await stageReached.Task.WaitAsync(
            TimeSpan.FromSeconds(10),
            TestContext.Current.CancellationToken);
        var lease = Assert.IsType<MariaDbMigrationLock>(lockProvider.AcquiredLease);
        ...
        Assert.True(detection.Elapsed <= MariaDbMigrationLock.LeaseLossDetectionBound);
    ...
    public async Task Two_real_orchestrators_execute_once_and_the_contender_rechecks_under_the_lock()
    {
        ...
        try
        {
            var serviceId = ServiceId.Parse($"mariadb-orchestrator-{Guid.NewGuid():N}");
            ...
            var secondExecutor = new RealStateExecutor(
                RequireConnectionString(), table, executionStarted, allowExecution);
            var firstProvider = new MariaDbMigrationLockProvider();
            var secondProvider = new MariaDbMigrationLockProvider(
                new AcquireSignallingOperations(
                    new MariaDbMigrationLockOperations(),
            ...
            var held = await Assert.ThrowsAsync<DatabaseMigrationLockException>(async () =>
                await new MariaDbMigrationLockProvider().AcquireAsync(
                    serviceId,
                    bootstrap,
                    TimeSpan.FromMilliseconds(250),
                    TestContext.Current.CancellationToken));
    ...
    {
        private readonly MariaDbMigrationLockProvider inner = new();
    ...
    private sealed class AcquireSignallingOperations(
        IMariaDbMigrationLockOperations inner,
        TaskCompletionSource acquireStarted) : IMariaDbMigrationLockOperations
    {
    ...
    private sealed class AcquireSignallingSession(
        IMariaDbMigrationLockSession inner,
        TaskCompletionSource acquireStarted) : IMariaDbMigrationLockSession
    {
...
{
    private const string DefaultImage = "mariadb:11.4.8";
    private const string ImageVariable = "SERVICEMANTLE_MARIADB_IMAGE";
