...
以下条款用于发现任务、审查交付和修复意见阶段，落实本文件及 `CONTRIBUTING.md` 的既有政策；
不改变任务范围、验收要求或 review 轮次预算，不要求历史 issue 批量重写，也不把工作流接入检查
设为每次任务执行的前置步骤。

### 授权与临时环境

- 分析、文档修改、代码修改、提交、推送、创建 PR 和合并，分别按用户当前请求及会话中仍有效的
  授权执行；不从技能名称额外推定未请求的发布或合并授权，也不因阶段或会话切换重复索取已有授权。
- 未获外部写入授权时，先完成已授权的本地工作并给出可审阅差异及交接记录，不代替用户发布。
- 本工作流的临时环境只清理本任务创建或已获授权清理的部分，并先确认没有未保存工作、用户改动
  或仍需保留的独有提交；无法确认时保留并说明。合并后的分支与 worktree 清理继续按下文流程执行。

### 修复与复审交接

- 按上文 review 分类和轮次预算固定待修清单，为每项记录依据、对应验收或保证、完成条件及状态，
  区分“待修”“已修复待复审”和“复审通过”。新意见先分类，不自动扩张原清单或实现范围。
- 完成必要验证后，按授权提交并推送原 PR 分支，记录修复提交、逐条验证证据和剩余意见，再交回
  复审；没有新证据或相关代码变化时，不反复修改已解决项。推送完成不代表复审通过或任务交付。
- 合并前核对平台当前适用的必需检查、审批和分支保护，不用本地验证或 review 轮次预算绕过门禁。
  对影响当前验收或合并的真实争议，记录争议依据、负责决策的角色、下一步与解除条件；未解决时
  保留对应状态，继续处理其他独立且已授权的工作。

### 持续交接记录

切换任务或压缩上下文前，在既有 tracker、原 task 或已授权的记录位置补齐：task/PR 完整链接、
实现仓库、目标分支、base/head 提交、适用规则与审计证据位置、逐条验收结果、意见分类及 review
轮次、已解决意见、已接受边界、剩余阻塞、下一步及解除条件。沿用同一份当前记录，不新建第二份
权威队列；未知项标为未知，不适用项说明理由，不推定通过或将轮次归零。没有可写位置或写入授权
时，在输出中提供完整交接记录；恢复工作时先核对相关变化，再复用仍有效的证据。
